# python rssi_logger.py --runs 20 -w givat_ram/120.0/rssi_log.csv -d 60
#!/usr/bin/env python3
import asyncio
import logging
import argparse
...
async def scan_for_peers(scan_duration: int, log_filename: str, runs: int, start_run: int):
    ...
    def detection_callback(device: BLEDevice, advertisement_data: AdvertisementData):
        ...
        if is_trusted_peer(device_name):
            timestamp = time.time()  # Use time.time() for a standard Unix timestamp
    ...
    for i, run in enumerate(range(start_run, start_run + runs)):
        print(f"🔍 Starting scan {i+1}/{runs} (run number {run}) for {scan_duration} seconds...")
        lines: list[str] = []
        ...
        # Extract RSSI values from this run's measurements
        run_rssi_values = []
        if lines:
            try:
        ...
        with open(full_filepath, 'a') as f:
            f.write(f"# Run {run} Start Timestamp: {run_start_timestamp}\n")
            f.write(f"# Run {run} End Timestamp: {run_end_timestamp}\n")
        ...
        if run_rssi_values:
            ...
            print(f"🤝 Run {run} complete. Samples: {count}, Mean RSSI: {mean:.2f} ± {std:.2f} dBm.")
        else:
            print(f"🤝 Run {run} complete. No trusted advertisements found.")
    elapsed_time = time.time() - start_time
    print(f"⏱️ Total elapsed time: {elapsed_time:.2f} seconds, total ads: {total_count}")
