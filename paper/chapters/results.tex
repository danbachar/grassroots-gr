...
To better understand how fast messages are delivered in different topologies, we generated successful delivery under time threshold heatmaps for different communication ranges and max node degrees in figure~\ref{figures/range100_maxdeg5_inter_thresholds_10s_60s.png}, which provide the full image together with the topological matrices introduced in subsection~\ref{subsec:simulations}, as to how messages are exchanged in a specific topology.
...
The difference between the two matrices is the time threshold used: for the top plot a tight threshold of 10 seconds was used, while a more relaxed 60s threshold was used for the bottom plot. As expected, the delivery probability improved by about 3x from 4.6\% to 13.8\%.
\begin{figure}[htbp]
    \centering
